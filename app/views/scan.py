...
async def process_single_video(request):
    """Process a single video and store it in the database.

    Args:
        request: The HTTP request object containing the video link.

    Returns:
        HttpResponseRedirect: A redirect to the home page with a success or error message.

    Raises:
        Exception: For any errors during the video processing.
    """
    try:
        from django.contrib import messages
        from django.shortcuts import redirect
        ...
        if not video_link:
            logger.error("Missing video link")
            messages.error(request, "Please enter a valid YouTube video link.")
            return redirect("app:home")

        logger.info("Starting single video processing", video_link=video_link)

        # Get YouTube scraper instance
        scraper = get_youtube_scraper()

        # Process the single video without channel dependency
        video_metadata, video_chunks = await scraper.scrape_single_video(
            video_link=video_link,
            channel_id=None  # No channel dependency
        )

        if not video_metadata:
            logger.error("Failed to process video", video_link=video_link)
            messages.error(request, "Could not process the video. Please check the link and try again.")
            return redirect("app:home")

        # Add chunks to vector database without channel dependency
        if video_chunks:
        ...
        logger.error("Error during single video processing", video_link=video_link, error=e, traceback=traceback.format_exc())
