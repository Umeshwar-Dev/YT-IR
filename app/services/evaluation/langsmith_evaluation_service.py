...
class LangsmithEvaluationService:
    ...
    def _parse_graph_output_to_example(self, graph_output: InputAgentState):
        ...
        return {
            "messages": str([str(msg) for msg in graph_output.get("messages", [])]),
            "user_information": str(graph_output.get("user")),
            "channel_information": str(graph_output.get("channel")),
            "answer": graph_output.get("messages", [])[-1].content if graph_output.get("messages") else "",
    ...
    async def add_example(
        ...
        try:
            ...
            channel_info = parsed_output["channel_information"].pretty_str_sync() if parsed_output["channel_information"] else "No channel (single video mode)"
            output_example = {
    ...
    def add_example_sync(self, graph_output: InputAgentState):
        ...
        try:
            ...
            input_example = {
                "messages": str([str(msg) for msg in parsed_output["messages"]]),
