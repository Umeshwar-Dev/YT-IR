...
class VectorDatabaseTools:
    ...
    @classmethod
    async def _get_video_data(cls, video_ids: List[str], channel_id: Optional[str] = None) -> dict[str, dict[str, Any]]:
        """Fetch video data efficiently.

        Retrieves video information for given video IDs using a single database query
        with optimized joins.

        Args:
            video_ids: List of video IDs to fetch data for.
            channel_id: Optional channel ID for context (not used in query but kept for compatibility)
        ...
        """
        # Single efficient query using values; works for both with/without channel
    ...
    @classmethod
    async def similarity_videos_search(cls, query: str, channel_id: Optional[str] = None, **kwargs) -> QueryVectorStoreResponse:
        ...
        Args:
            ...
            channel_id: YouTube channel ID to restrict the search (optional for single video mode)
            **kwargs: Additional arguments to pass to the search

        Returns:
            QueryVectorStoreResponse: Object containing relevant chunks and videos matching to query

        Raises:
            ConnectionError: If unable to connect to the vector database
            ValueError: If query or channel_id is invalid
        """
        ...
        # Extract unique video IDs - ONLY from chunks that have video_id
        # Debug: Log metadata of first few results
        if combined_results:
            first_result = combined_results[0]
            logger.info("Sample metadata from search results", 
                       sample_metadata=first_result.metadata,
                       total_results=len(combined_results),
                       available_keys=list(first_result.metadata.keys()))
        
        # Filter chunks to only include those with video_id
        chunks_with_video_id = [r for r in combined_results if r.metadata.get("video_id")]
        logger.info("Filtered chunks with video_id", 
                   total_chunks=len(combined_results), 
                   chunks_with_video_id=len(chunks_with_video_id))
        
        if not chunks_with_video_id:
            logger.warning("No chunks with video_id found in search results", query=query, channel_id=channel_id)
            return QueryVectorStoreResponse(chunks=[], videos=[])
        
        video_ids = list({r.metadata.get("video_id") for r in chunks_with_video_id if r.metadata.get("video_id")})
        if not video_ids:
            # Try alternative keys
            all_video_ids = []
            for r in combined_results:
                for key in r.metadata:
                    if 'video' in key.lower() and 'id' in key.lower():
                        all_video_ids.append(r.metadata[key])
            
            if all_video_ids:
                video_ids = list(set(all_video_ids))
                logger.info("Found video IDs with alternative keys", video_ids=video_ids[:5], alternative_keys_used=True)
            else:
        ...
        # Fetch video data efficiently
        video_map = await cls._get_video_data(video_ids, channel_id)
