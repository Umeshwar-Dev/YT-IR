...
class YoutubeScraper(BaseYoutubeScraper):
    ...
    def validate_video_link(video_link: str) -> str:
        """Validate and extract video ID from a YouTube video link.
        ...
        """
        return validate_video_link(video_link)

    async def scrape_single_video(self, video_link: str, channel_id: Optional[str] = None) -> Tuple[Optional[Dict], List[Dict]]:
        """Scrape a single video and its transcript.

        This method processes a single video by URL, extracting metadata and transcript
        without needing to scan an entire channel.

        Args:
            video_link: The YouTube video URL to scrape.
            channel_id: Optional channel ID to associate the video with.

        Returns:
            Tuple[Optional[Dict], List[Dict]]: A tuple containing:
                - Video metadata dictionary or None if failed
                - List of transcript chunk dictionaries

        Note:
            This method avoids the "too many requests" error by processing only one video
            instead of scanning entire channels.
        """
        try:
            logger.info("Starting single video scrape", video_link=video_link)

            # Validate video link and extract video ID
            video_id = self.validate_video_link(video_link)

            # Create a minimal video data structure for the scraper
            video_data = {
                "videoId": video_id,
                "title": {"runs": [{"text": f"Video {video_id}"}]},
                "descriptionSnippet": {"runs": [{"text": ""}]},
                "thumbnail": {"thumbnails": [{"url": f"https://i.ytimg.com/vi/{video_id}/default.jpg"}]},
            ...
            video_metadata, video_chunks = await self.video_scraper.process_single_video(
                video_data, self.transcript_scraper
            )
            ...
            logger.error("Single video scrape failed", video_link=video_link, error=e, traceback=format_exc())
            return None, []
